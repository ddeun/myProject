...
<body>
	...
	<form name="register" method="post" action="update">
		...
		<p> 아이디 : ${edit.rid}
		<p> 비밀번호 : <input type="password" name="rpasswd" value="${edit.rpasswd}">
		...
		<p> 성별 : <input type="radio" name="rgender" value="남성" 
					<c:if test="${edit.rgender=='남성'}">checked</c:if> disabled>남성
				  <input type="radio" name="rgender" value="여성"
				  	<c:if test="${edit.rgender=='여성'}">checked</c:if> disabled>여성
		<p> 취미 : 독서<input type="checkbox" name="rhobby" value="독서" 
					<c:if test="${fn:contains(edit.rhobby,'독서')}">checked</c:if>>
				  운동<input type="checkbox" name="rhobby" value="운동"
				    <c:if test="${fn:contains(edit.rhobby,'운동')}">checked</c:if>>
				  영화<input type="checkbox" name="rhobby" value="영화"
				    <c:if test="${fn:contains(edit.rhobby,'영화')}">checked</c:if>>
